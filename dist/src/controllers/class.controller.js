  ...
  try {
    if ((!student_id || mongoose_1.default.isValidObjectId(student_id)) && (!course_id || mongoose_1.default.isValidObjectId(course_id))) {
      ...
      if (course_id) {
        ...
        if (result.length === 0) return res.status(404).json(new httpResponseData_1.default(response_config_1.RESPONSE_CONFIG.MESSAGE.CLASS.NOT_FOUND, 404));
        res
          .status(200)
          .json(
            new httpResponseData_1.default(
              response_config_1.RESPONSE_CONFIG.MESSAGE.CLASS.FOUND_SUCCESS,
              200,
              result,
              result.length,
              num.length,
        ...
        if (result.length === 0) return res.status(404).json(new httpException_1.default(response_config_1.RESPONSE_CONFIG.MESSAGE.CLASS.NOT_FOUND, 404));
        res
          .status(200)
          .json(
            new httpResponseData_1.default(
...
const GetClassInfo = async (req, res) => {
  ...
  try {
    ...
    if (!exist) return res.status(404).send(new httpResponseData_1.default(response_config_1.RESPONSE_CONFIG.MESSAGE.CLASS.NOT_FOUND, 404));
...
const UpdateStatusStudentInClass = async (req, res) => {
  ...
  try {
    ...
    if (!_class || !_student) return res.status(404).send(new httpResponseData_1.default(response_config_1.RESPONSE_CONFIG.MESSAGE.CLASS.NOT_FOUND, 404));
