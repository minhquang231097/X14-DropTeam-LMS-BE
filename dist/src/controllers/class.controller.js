    ...
    try {
        if ((!student_id || mongoose_1.default.isValidObjectId(student_id)) && (!course_id || mongoose_1.default.isValidObjectId(course_id))) {
            ...
            if (course_id) {
                ...
                const num = await class_service_1.default.GetClassByMentorId(mentor_id);
                let result;
                if (p === undefined && l === undefined) {
                    result = await class_service_1.default.GetClassByMentorId(mentor_id, 1, 10);
                }
                else {
                    result = await class_service_1.default.GetClassByMentorId(mentor_id, p, l);
                }
                if (result.length === 0)
                    return res.status(404).json(new httpException_1.default(response_config_1.RESPONSE_CONFIG.MESSAGE.CLASS.NOT_FOUND, 404));
                res
...
const UpdateClass = async (req, res) => {
    ...
    try {
        const exist = await class_service_1.default.GetClassById(id);
        if (exist) {
            await class_service_1.default.UpdateOneClass(id, update);
            const newClass = await class_service_1.default.GetClassById(id);
            return res.status(200).json(new httpResponseData_1.default(response_config_1.RESPONSE_CONFIG.MESSAGE.CLASS.UPDATE_SUCCESS, 200, newClass));
        }
        return res.status(404).send(new httpResponseData_1.default(response_config_1.RESPONSE_CONFIG.MESSAGE.CLASS.NOT_FOUND, 404));
    }
    catch (error) {
...
const UpdateStatusStudentInClass = async (req, res) => {
    ...
    try {
        const exist = await class_student_service_1.default.CheckStudentExistInClass(student_id, class_id);
        if (exist) {
            await class_student_service_1.default.UpdateStatusStudentInClass(payload);
            const newUpdate = await class_student_service_1.default.GetStudentInClassByStudentId(student_id);
            return res.status(200).json(new httpResponseData_1.default(response_config_1.RESPONSE_CONFIG.MESSAGE.CLASS.UPDATE_SUCCESS, 200, newUpdate));
        }
        return res.status(404).send(new httpResponseData_1.default(response_config_1.RESPONSE_CONFIG.MESSAGE.CLASS.NOT_FOUND, 404));
    }
    catch (error) {
