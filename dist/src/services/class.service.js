...
const CreateOneClass = async (email_mentor, workplace_code, course_code, payload) => {
    ...
    if (!_class) {
        ...
        const session_per_week = schedule.length;
        let arrSch = [];
        ...
        const date_end = calculateEndDate(start_at, total_session, arrSch);
...
function calculateEndDate(startDate, totalSessions, schedule) {
    ...
    const end = new Date(startDate.getTime() + sessionCount * 24 * 60 * 60 * 1000);
    return end;
}
