...
const CreateFeedback = async (course_code: string, email_student: string, payload: any) => {
    ...
    const newFeedback = await feedbackRepository.Create({ ...payload, course: _course?._id, student: _student._id })
    return newFeedback
}
...
export default { CreateFeedback, GetFeedbackById, DeleteFeedbackByCondition, DeleteFeedbackById, UpdateFeedback, GetFeedbackByCondition, GetFeedbackByCourseCode, GetFeedbackByEmailStudent }