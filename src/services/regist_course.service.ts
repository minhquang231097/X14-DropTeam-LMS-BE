...
const GetRegistByCourseCode = async (code: string) => {
    const _course = await courseService.GetCourseByCode(code)
    return await registCourseRepository.FindRegistbyCourseId(_course?._id)
}
...
export default { DeleteRegist, UpdateRegist, CreateRegistCourse, GetRegistByCourseCode, GetRegistByWorkplaceCode, GetRegistByEmailStudent }