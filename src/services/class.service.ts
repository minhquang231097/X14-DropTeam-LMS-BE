...
const CreateOneClass = async (
  ...
  const { start_at, session_per_week, schedule, total_session, class_code } =
    payload;
  const [_mentor, _workplace, _course, _code] = await Promise.all([
    ...
    classRepository.FindByCondition({ class_code: class_code }),
  ]);
  ...
  if (!_code) {
    ...
    const day_start = new Date(start_at).getDay() - 1;
    ...
    let date_end;
    if (
      (total_session % Number(session_per_week)) - 1 + start <
        schedule.length &&
      total_session % Number(session_per_week) > 0
    ) {
      week_end = weekStart + number_week;
      const day =
        schedule[(total_session % Number(session_per_week)) - 1 + start];
      date_end = moment()
        .week(week_end)
        .day(day + 1)
        .toDate();
      ...
      start === 0 &&
      (total_session % Number(session_per_week)) - 1 + start > schedule.length
    ) {
      ...
      start > 0 &&
      (total_session % Number(session_per_week)) + start - 1 > schedule.length
    ) {
      week_end = weekStart + number_week + 1;
      const day =
        schedule[
