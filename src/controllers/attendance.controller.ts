...
const UpdateAttendance = async (req: Request, res: Response) => {
  ...
  try {
    const result = await attendanceService.UpdateAttendance(id as string, update);
    if (!result) {
      res.status(404).send(new HttpException(RESPONSE_CONFIG.MESSAGE.ATTENDANCE.NOT_FOUND, 404));
    }
    res.status(200).json(new HttpResponseData(RESPONSE_CONFIG.MESSAGE.ATTENDANCE.UPDATE_SUCCESS, 200, result));
  } catch (error: any) {
...
const DeleteAttendance = async (req: Request, res: Response) => {
  const { id } = req.params;
  try {
    const result = await attendanceService.DeleteAttendanceById(id as string);
    if (!result) {
