...
const GetFeedback = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (course_id) {
      ...
      if (!result) return res.status(404).send(new HttpException(RESPONSE_CONFIG.MESSAGE.FEEDBACK.NOT_FOUND, 404));
      res
        .status(200)
      ...
      if (!result) return res.status(404).send(new HttpException(RESPONSE_CONFIG.MESSAGE.FEEDBACK.NOT_FOUND, 404));
      res.json(new HttpResponseData(RESPONSE_CONFIG.MESSAGE.FEEDBACK.FOUND_SUCCESS, 200, result));
    }
  } catch (error: any) {
...
const UpdateFeedback = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(200).json(new HttpResponseData(RESPONSE_CONFIG.MESSAGE.FEEDBACK.UPDATE_SUCCESS, 200));
  } catch (error) {
