...
const GetCourse = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (search) {
      ...
      res
        .status(200)
        .json(new HttpResponseData(RESPONSE_CONFIG.MESSAGE.COURSE.FOUND_SUCCESS, 200, result, result.length, countDoc, 1, Math.ceil(countDoc / LIMIT_PAGE_COURSE)));
    }
  } catch (error) {
...
const GetCourseInfo = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(404).send(new HttpException(RESPONSE_CONFIG.MESSAGE.COURSE.WRONG, 404, error.message));
  }
};
